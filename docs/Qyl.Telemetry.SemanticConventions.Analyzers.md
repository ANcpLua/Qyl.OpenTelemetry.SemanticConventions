...
The property is exposed via the analyzer NuGet's `buildTransitive/Qyl.Telemetry.SemanticConventions.Analyzers.props`, which appends the matching editorconfig from `buildTransitive/editorconfig/` to `$(EditorConfigFiles)` on consumer restore. The name is deliberately not bare `<AnalysisMode>` — that property is owned by `Microsoft.CodeAnalysis.NetAnalyzers` and clashing would force consumers into one-or-the-other choices.
...
- [SARIF rule manifest](Qyl.Telemetry.SemanticConventions.Analyzers.sarif) — SARIF v2.1.0 catalog of every `QYL00xx` rule (id, name, severity, category, helpUri). Consume from Sonar bridges, GitHub Advanced Security uploads, or IDE rule-catalog tools.
- [`AnalyzerReleases.Shipped.md`](../src/Qyl.Telemetry.SemanticConventions.Analyzers/AnalyzerReleases.Shipped.md) — release-tracking manifest with `ClassName, [Documentation](url)` attribution per Microsoft NetAnalyzers convention.
