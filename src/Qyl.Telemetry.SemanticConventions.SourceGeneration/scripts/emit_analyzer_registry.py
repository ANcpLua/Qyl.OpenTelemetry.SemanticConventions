...
DEFAULT_QYL_REGISTRY = SCRIPT_DIR.parent / "Resources" / "qyl-registry.json"
DEFAULT_OUTPUT = (
    REPO_ROOT
    / "src"
...
def generate(registry: dict, qyl_registry: dict) -> str:
    attributes = sorted(
        list(registry["catalog"]) + list(qyl_registry.get("attributes", [])),
    ...
    metric_names = sorted(
        ...
        | {metric["metric_name"] for metric in qyl_registry.get("metrics", [])}
    )
    scope_names = sorted(set(qyl_registry.get("scope_names", [])))
    event_names = sorted(
        {
            group["event_name"]
            for group in registry["groups"]
            if group.get("type") == "event" and group.get("event_name")
        }
        | set(qyl_registry.get("event_names", []))
    )
    ...
    lines = [
        ...
        "// merged with qyl-registry.json (qyl-owned vocabulary, loop 1).",
    ...
    lines.extend([
        ...
        f"        new({render_string_array(metric_names)}, StringComparer.Ordinal);",
        "",
        "    private static readonly HashSet<string> KnownScopeNames =",
        f"        new({render_string_array(scope_names)}, StringComparer.Ordinal);",
        "",
        "    private static readonly HashSet<string> KnownEventNames =",
        f"        new({render_string_array(event_names)}, StringComparer.Ordinal);",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--qyl-registry", type=Path, default=DEFAULT_QYL_REGISTRY)
    ...
    expected = generate(
        ...
        json.loads(args.qyl_registry.read_text()),
    )
    if args.check:
