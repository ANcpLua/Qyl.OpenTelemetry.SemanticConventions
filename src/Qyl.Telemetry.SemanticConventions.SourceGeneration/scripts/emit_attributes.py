...
Reads the embedded resolved-registry.json PLUS the qyl-owned qyl-registry.json
and regenerates, per root namespace, `{Root}Attributes.g.cs` for both the stable
and the incubating NuGet packages, plus `Names/QylTelemetryNames.g.cs` for the
qyl-owned scope and bridge-event names.
...
STABLE_ROOT = os.path.join(SRC, "Qyl.Telemetry.SemanticConventions", "Attributes")
...
def emit_names_file(qyl_registry) -> str:
    scopes = _name_members(qyl_registry.get("scope_names", []), "scope name")
    events = _name_members(qyl_registry.get("event_names", []), "event name")
...
# --------------------------------------------------------------------------- #
def load_qyl_registry():
    with open(QYL_JSON_PATH, encoding="utf-8") as f:
        return json.load(f)
...
def load_groups():
    ...
    # The qyl-owned vocabulary is catalog content, not a second surface: it is
    # tagged with its source registry and then filtered, grouped and emitted by
    # exactly the same rules as the upstream entries.
    for attr in load_qyl_registry().get("attributes", []):
        entry = dict(attr)
        entry["source_registry"] = QYL_SOURCE_REGISTRY
